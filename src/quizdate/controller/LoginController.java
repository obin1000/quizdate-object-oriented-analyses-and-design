...
public class LoginController {
    ...
    public void loginButtonPressed(ActionEvent event) {
        ...
        if ((userId = UserRepository.getInstance().checkLoginInformation(txt_username.getText(), txt_password.getText())) != 0) {
            //CHECK IF LOGIN DATA IS CORRECT, THEN DENY OR LOGIN.
            System.out.println("logindetails correct!");
            System.out.println(userId);
            MainController.getMainController().setCurrentUser(UserRepository.getInstance().get(userId), userId); //Sets the user to singleton static MainController if a userId is found.
